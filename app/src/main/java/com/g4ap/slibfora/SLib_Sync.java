...
public class SLib_Sync {

	// warning change it only chang phone
	private String sys_sync_type = "2";
	private String sys_sync_device = "9999_ERROR";
	...
	public void Init()
	{
		SQLiteDatabase db = SQLiteDatabase.openDatabase(SLib.m_SLibDBPath, null, SQLiteDatabase.OPEN_READONLY);
		Cursor c = db.rawQuery( "SELECT strdata FROM t_setting WHERE id=0", null );
		if ( c.getCount() == 1 )
		{
			c.moveToFirst();
	...
	public void syncAndroidCallhisDB( Activity act )
	{
		...
		SQLiteDatabase db = SQLiteDatabase.openDatabase(SLib.getInstance().m_SLibDBPath, null, SQLiteDatabase.OPEN_READWRITE);
	...
	{
		retSyncStateA ret = new retSyncStateA();
        ...
        c.close();
        ret.m_CallhisSys = maxCallhisSys;
		
        // load sms sys
        long maxSMSSys = 0;
		Uri uriSMS = Uri.parse("content://sms");
        c = act.getContentResolver().query(uriSMS, null, null, null, null);
        if ( c.getCount() > 0 )
        {
        ...
        c.close();
        ret.m_SMSSys = maxSMSSys;
        
        // load callhis slib
		SQLiteDatabase db = SQLiteDatabase.openDatabase(SLib.getInstance().m_SLibDBPath, null, SQLiteDatabase.OPEN_READONLY);
		c = db.rawQuery( "SELECT MAX(a_id) AS maxaid FROM t_callhis WHERE sync_type = ? AND sync_device LIKE ?", new String[]{sys_sync_type,sys_sync_device} );
        if ( c.getCount() > 0 )
        {
        	c.moveToFirst();
			if ( !c.isNull(c.getColumnIndex("maxaid")) )
			{
				ret.m_CallhisSLib = c.getLong(c.getColumnIndex("maxaid"));
			}
			else
			{
				ret.m_CallhisSLib = 0;
        ...
        // load sms slib
		c = db.rawQuery( "SELECT MAX(a_id) AS maxaid FROM t_sms WHERE sync_type = ? AND sync_device LIKE ?", new String[]{sys_sync_type,sys_sync_device} );
        if ( c.getCount() > 0 )
        {
        	c.moveToFirst();
			if ( !c.isNull(c.getColumnIndex("maxaid")) )
			{
				ret.m_SMSSLib = c.getLong(c.getColumnIndex("maxaid"));
			}
			else
			{
				ret.m_SMSSLib = 0;
		...
		ret.sync_type = sys_sync_type;
		ret.sync_device = sys_sync_device;
	...
	public String quaryAddrNameByTel( String tel )
	{
		...
		SQLiteDatabase db = SQLiteDatabase.openDatabase(SLib.getInstance().m_SLibDBPath, null, SQLiteDatabase.OPEN_READONLY);
		Cursor c = db.rawQuery( "SELECT name FROM t_addr WHERE tel LIKE ?", new String[]{ strFix } );
        ...
        {
        	c.moveToFirst();
        	String ret = c.getString( c.getColumnIndex("name") );
        	c.close();
        	db.close();
        	return ret;
        }
